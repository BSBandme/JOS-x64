...
//	for(i = PML4(UTOP); i < PGSIZE / 512/*sizeof (pml4e_t)*/; i++) {
//	e->env_pml4e[0] = boot_pml4e[0];
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	void *en = ROUNDUP(va + len, PGSIZE), *be = ROUNDDOWN(va, PGSIZE);	
	for(; be < en; be += PGSIZE) {
		struct PageInfo *pp = page_alloc(1);
		if(pp) {
			pp->pp_ref++;
			int flag = page_insert(e->env_pml4e, pp, be, PTE_U | PTE_W | PTE_P);
...
//		}
	...
	return;
bad: 
	panic("load ELF failed, Not valid ELF!");
}  
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	if(flag) 
		panic("Env create failed: %e", flag);
...
env_run(struct Env *e)
{
	...
	if(e->env_status != ENV_RUNNABLE)
		panic("Env can't' be run");
	curenv = e;
	e->env_status = ENV_RUNNING;
	e->env_runs++;
	cprintf("%016x %016x\n", e->env_cr3, PADDR(e->env_pml4e));
	lcr3(e->env_cr3);
	//cprintf("%016x %016x %016x\n", envs[0]->env_cr3, envs[0]->env_pml4e, envs[0]->env_runs);
	env_pop_tf(&e->env_tf);
}
