...
i386_detect_memory(void)
{
	...
	cprintf("Physical memory: %uM available, base = %uK, extended = %uK, npages = %d\n, page start = %d\n",
		...
		npages,
		(uint32_t)PADDR(BOOT_PAGE_TABLE_START) / 1024);
...
x64_vm_init(void)
{
	...
	// Your code goes here:
	size_t roundSize= ROUNDUP(npages * sizeof (struct PageInfo), PGSIZE);
	boot_map_region(pml4e, UPAGES, roundSize, PADDR(pages), PTE_U|PTE_P);
	...
	// LAB 3: Your code here.
	roundSize = ROUNDUP(sizeof (struct Env) * NENV, PGSIZE);
	boot_map_region(pml4e, UENVS, roundSize, PADDR(envs), PTE_U|PTE_P);
	...
	roundSize=(size_t)ROUNDUP(KSTKSIZE,PGSIZE);
	boot_map_region(pml4e, KSTACKTOP-KSTKSIZE, roundSize, PADDR(bootstack), PTE_W|PTE_P);
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here: 
	roundSize=(size_t) ROUNDUP (npages*PGSIZE, PGSIZE);
	boot_map_region(pml4e, KERNBASE, roundSize, 0, PTE_W|PTE_P);
...
page_alloc(int alloc_flags)
{
	...
	result->pp_link = 0;
	void *addr = page2kva(result);

	if(alloc_flags & ALLOC_ZERO) 
		 memset(addr, 0, PGSIZE * sizeof(char));
...
//	cprintf("page_free\n");
	if(pp->pp_ref)
	 	panic("page_ree when pp_ref is NOT 0");
	if(pp->pp_link) 
		panic("page_free when page is already free");
...
{
//	cprintf("pml4e_walk\n");
	pdpe_t * pt = KADDR(PADDR(pml4e) + PML4(va)*sizeof(physaddr_t));

	void *pt_kva;
    if (pt != NULL && (*pt & PTE_P)) {
    	pt_kva = pdpe_walk((pdpe_t*)KADDR(PTE_ADDR (*pt)), va, create);
		return (pte_t*) pt_kva;
    }
	if(!create) 
		return NULL;
	...
	*pt = (pml4e_t)page2pa(newPage);
	*pt |= PTE_P | PTE_U | PTE_W;
	newPage->pp_ref++;

	pte_t *ans = pdpe_walk((pdpe_t*)KADDR(PTE_ADDR(*pt)), va, create);
	if(ans == NULL) {
		*pt = 0;
		page_decref(newPage);
	}
...
pdpe_walk(pdpe_t *pdpe,const void *va,int create){
	pdpe_t * pt = KADDR(PADDR(pdpe) + PDPE(va)*sizeof(physaddr_t));
	void *pt_kva;

    if (pt != NULL && (*pt & PTE_P)) {
    	pt_kva = pgdir_walk((pde_t*)KADDR(PTE_ADDR (*pt)), va, create);
		return (pte_t*) pt_kva;
    }
	if(!create) 
		return NULL;
	...
	*pt = (pdpe_t)page2pa(newPage);
	*pt |= PTE_P | PTE_U | PTE_W;
	newPage->pp_ref++;

	pte_t *ans = pgdir_walk((pde_t*)KADDR(PTE_ADDR(*pt)), va, create);
	if(ans == NULL) {
		*pt = 0;
		page_decref(newPage);
	}
...
{
	// Fill this function in
	pde_t * pt = KADDR(PADDR(pgdir) + PDX(va)*sizeof(physaddr_t));

	void *pt_kva;

    if (pt != NULL && (*pt & PTE_P)) {
    	pt_kva = (void*)KADDR(PTE_ADDR (*pt) + PTX (va)*sizeof(physaddr_t));
		return (pte_t*) pt_kva;
    }
	...
	*pt = (pde_t)page2pa(newPage);
	*pt |= PTE_P |PTE_U|PTE_W;
	newPage->pp_ref++;
//	if(va == (const void *)KERNBASE)
//		cprintf("%016x|%016x|%016x|%016x\n", pgdir, &pgdir[32], pt, *pt);

	return KADDR(PTE_ADDR(*pt) + PTX(va)*sizeof(physaddr_t));
}
...
//	pde_t *temp = NULL;
	for(i = la; i < la + size; i += PGSIZE, pa += PGSIZE) {
		...
		const void *j = (void *)i;
		pte_t *val = pml4e_walk(pml4e, j, 1);
/*		pml4e_t *tpml4e = KADDR(PADDR(pml4e) + PML4(j)*sizeof(physaddr_t));
		pdpe_t *tpdpe = KADDR(PTE_ADDR(*tpml4e) + PDPE(j)*sizeof(physaddr_t));
		pde_t *tpde = KADDR(PTE_ADDR(*tpdpe) + PDX(j)*sizeof(physaddr_t));*/

		*val = pa | perm | PTE_P;
//		if(i == la && perm){
//			 cprintf("%016x|", *tpml4e);
//		}
/*		*tpml4e = PTE_ADDR(*tpml4e) | perm | PTE_P;*/
//		if(i == la && perm){
//			 cprintf("%016x\n", *tpml4e);
//		}

/*		*tpdpe = PTE_ADDR(*tpdpe) | perm | PTE_P;
		*tpde = PTE_ADDR(*tpde) | perm | PTE_P;*/
//		if((void *)i == (void *)KERNBASE) {
//			temp = tpde;
//		}
//		if(temp != NULL) {
//			cprintf("%016x|%016x|%d || ", *temp, temp, PDX(j));
//			pdpe_t *t = KADDR(PTE_ADDR(boot_pml4e[1]));
//			pde_t *pgdir = KADDR(PTE_ADDR(t[0]));
//			cprintf("%016x|%016x|%016x|%016x|%016x|%016x\n", pml4e, boot_pml4e, *tpml4e, boot_pml4e[1], *tpdpe, pgdir[32]);
...
{
	pte_t *val = pml4e_walk(pml4e, va, 1);
	if(val == NULL) {
		return -E_NO_MEM;
	}

	if((*val) & PTE_P) {
		if(page2pa(pp) != PTE_ADDR(*val))
...
/*	pml4e_t *tpml4e = KADDR(PADDR(pml4e) + PML4(va)*sizeof(physaddr_t));
	pdpe_t *tpdpe = KADDR(PTE_ADDR(*tpml4e) + PDPE(va)*sizeof(physaddr_t));
	pde_t *tpde = KADDR(PTE_ADDR(*tpdpe) + PDX(va)*sizeof(physaddr_t));*/

	*val = page2pa(pp) | perm | PTE_P;
/*	*tpml4e = PTE_ADDR(*tpml4e) | perm | PTE_P;
	*tpdpe = PTE_ADDR(*tpdpe) | perm | PTE_P;
	*tpde = PTE_ADDR(*tpde) | perm | PTE_P;*/


//	 *val = page2pa(pp) | (uint64_t)perm | (uint64_t)PTE_P;
	 pp->pp_ref++;
...
{
	pte_t * ptePointer = pml4e_walk (pml4e, va, 0);

 	if (pte_store != 0) {
		 *pte_store = ptePointer;
	}

 	if(ptePointer != NULL && (*ptePointer & PTE_P)){
		 return pa2page (PTE_ADDR (*ptePointer));
	}
...
page_remove(pml4e_t *pml4e, void *va)
{
	// Fill this function in
	pte_t *val = NULL;
	struct PageInfo *p = page_lookup(pml4e, va, &val);
	
	if(val == NULL || !(*val & PTE_P))
		return;
	page_decref(p);
	*val = 0;
	tlb_invalidate(pml4e, va);
...
//	cprintf("%016x ", (uint64_t)page_free_list);
	page_free(pp0);
//	cprintf("%016x %016x\n", (uint64_t)pp0->pp_link, (uint64_t)pp0);
	page_free(pp1);
	page_free(pp2);
	pp0 = pp1 = pp2 = 0;
	assert((pp0 = page_alloc(0)));
...
check_boot_pml4e(pml4e_t *pml4e)
{
	...
	for (i = 0; i < n; i += PGSIZE) {
		//cprintf("%x %x %x %x\n",i,check_va2pa(pml4e, UPAGES + i), PADDR(pages) + i, pml4e);
	...
	for (i = 0; i < NPDENTRIES; i++) {
		...
		default:
			if (i >= PDX(KERNBASE)) {
				if (pgdir[i] & PTE_P) {
	//				pte_t *val = pml4e_walk(pml4e, (const void *)KERNBASE, 0);
